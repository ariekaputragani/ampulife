export const illnessCatalog = [
  {
    id: "flu",
    name: "Flu",
    minAge: 0,
    maxAge: 999,
    baseSeverity: "mild",
    yearlyPenalty: { health: 3, happy: 2 },
    mortalityRisk: 0,
    chanceWeight: 100,
  },
  {
    id: "pertussis",
    name: "Batuk Rejan",
    minAge: 0,
    maxAge: 999,
    baseSeverity: "mild",
    yearlyPenalty: { health: 15, happy: 5 },
    mortalityRisk: 0.01,
    chanceWeight: 40,
  },
  {
    id: "measles",
    name: "Campak",
    minAge: 0,
    maxAge: 999,
    baseSeverity: "mild",
    yearlyPenalty: { health: 15, happy: 5 },
    mortalityRisk: 0.005,
    chanceWeight: 40,
  },
  {
    id: "dengue",
    name: "Demam Berdarah",
    minAge: 0,
    maxAge: 999,
    baseSeverity: "moderate",
    yearlyPenalty: { health: 15, happy: 10 },
    mortalityRisk: 0.02,
    chanceWeight: 30,
  },
  {
    id: "cancer",
    name: "Kanker",
    minAge: 0,
    maxAge: 999,
    baseSeverity: "severe",
    yearlyPenalty: { health: 35, happy: 15 }, // Adjusted based on legacy age-dependent logic (max penalty)
    mortalityRisk: 0.12,
    chanceWeight: 1,
  },
  {
    id: "coronavirus",
    name: "Coronavirus",
    minAge: 0,
    maxAge: 999,
    baseSeverity: "severe",
    yearlyPenalty: { health: 8, happy: 5 },
    mortalityRisk: 0.08,
    chanceWeight: 20,
  },
  {
    id: "alzheimer",
    name: "Alzheimer",
    minAge: 60,
    maxAge: 999,
    baseSeverity: "moderate",
    yearlyPenalty: { health: 8, happy: 5, smarts: 10 },
    mortalityRisk: 0.03,
    chanceWeight: 10,
  },
  {
    id: "epilepsy",
    name: "Epilepsi",
    minAge: 0,
    maxAge: 999,
    baseSeverity: "moderate",
    yearlyPenalty: { health: 5, happy: 3 },
    mortalityRisk: 0.01,
    chanceWeight: 3,
  },
  {
    id: "diarrhea",
    name: "Diare",
    minAge: 0,
    maxAge: 999,
    baseSeverity: "mild",
    yearlyPenalty: { health: 3, happy: 2 },
    mortalityRisk: 0.005,
    chanceWeight: 100,
  },
  {
    id: "diabetes",
    name: "Diabetes",
    minAge: 40,
    maxAge: 999,
    baseSeverity: "moderate",
    yearlyPenalty: { health: 3, happy: 2 },
    mortalityRisk: 0.02,
    chanceWeight: 15,
  },
  {
    id: "tuberculosis",
    name: "Tubercolosis",
    minAge: 18,
    maxAge: 999,
    baseSeverity: "moderate",
    yearlyPenalty: { health: 15, happy: 8 },
    mortalityRisk: 0.04,
    chanceWeight: 10,
  },
  {
    id: "heart_disease",
    name: "Penyakit Jantung",
    minAge: 60,
    maxAge: 999,
    baseSeverity: "severe",
    yearlyPenalty: { health: 5, happy: 5 },
    mortalityRisk: 0.1,
    chanceWeight: 8,
  },
];

export function getIllnessById(illnessId) {
  return illnessCatalog.find((item) => item.id === illnessId) ?? null;
}

export function pickIllnessByAge(age, rng = Math.random) {
  const filtered = illnessCatalog.filter((item) => age >= item.minAge && age <= item.maxAge);
  if (filtered.length === 0) {
    return null;
  }

  const totalWeight = filtered.reduce((sum, item) => sum + (item.chanceWeight || 10), 0);
  let randomVal = rng() * totalWeight;

  for (const item of filtered) {
    const weight = item.chanceWeight || 10;
    if (randomVal < weight) {
      return item;
    }
    randomVal -= weight;
  }

  return filtered[0] ?? null;
}
