...
export function ageUpYear(state, rng = Math.random) {
  ...
  if (next.jailActions) {
    next.jailActions.hasRioted = false;
    next.jailActions.hasAttemptedEscape = false;
  }
  ...
  next.relations = next.relations.map((relation) => {
    ...
    // Hubungan berkurang 8% tiap tahun jika di penjara (khusus pacar/suami/istri)
    if (next.legal.inJail && (relation.status === "partner" || relation.status === "spouse")) {
      passiveDecay -= 8;
    }
  ...
  // --- 1.2 AUTOMATIC BREAKUP / DIVORCE (Bond 0%) ---
  const toRemove = [];
  next.relations.forEach(rel => {
    if (rel.relationship <= 0 && (rel.status === "partner" || rel.status === "spouse")) {
      toRemove.push(rel);
    }
  });

  toRemove.forEach(rel => {
    if (rel.status === "spouse") {
      const splitPercent = 0.455 + (rng() * (0.555 - 0.455));
      const lostAmount = Math.floor(next.money * splitPercent);
      next.money -= lostAmount;
      if (next.family) next.family.isKB = false;

      const divorceMsg = `Saya bercerai dengan ${rel.name}. Harta dibagi rata sebesar Rp${lostAmount.toLocaleString("id-ID")} diberikan kepada mantan pasanganmu.`;
      pushLog(next, divorceMsg);
      pushNotification(next, { title: "Hubungan", message: divorceMsg, icon: "error" });
      otherEvents = true;
    } else {
      const breakupMsg = `Saya sudah putus dengan ${rel.name}.`;
      pushLog(next, breakupMsg);
      pushNotification(next, { title: "Hubungan", message: breakupMsg, icon: "error" });
      otherEvents = true;
    }
    next.relations = next.relations.filter(r => r.id !== rel.id);
  });
  ...
  myChildren.forEach(child => {
    let cost = 0;
    if (child.age <= 5) cost = 6_000_000;
    else if (child.age <= 12) cost = 5_000_000;
    else if (child.age <= 15) cost = 4_000_000;
    else if (child.age <= 22) cost = 3_000_000; // Termasuk kuliah jika masih tanggungan

    if (lifestyle === "mewah") cost *= 3;
    else if (lifestyle === "hemat") cost *= 0.5;
    // normal: tidak ada multiplier berlebihan
  ...
  // Childcare cost: biaya orang tua untuk merawat karakter (player as their child)
  // Berlaku selama player masih tinggal di rumah orang tua (termasuk saat kuliah)
  let childcareCost = 0;
  if (!next.profile.isIndependent && next.profile.livingWithParents !== false) {
    if (next.age <= 5) childcareCost = 6_000_000;         // Balita
    else if (next.age <= 12) childcareCost = 5_000_000;   // SD
    else if (next.age <= 15) childcareCost = 4_000_000;   // SMP
    else if (next.age <= 22) childcareCost = 3_000_000;   // SMA/Kuliah (masih di rumah)
    // Di atas 22: tidak ada biaya rawat (sudah mandiri)

    if (next.family.wealthStatus === "rich") childcareCost *= 3;
    else if (next.family.wealthStatus === "poor") childcareCost *= 0.5;
    // middle: tidak ada multiplier berlebihan
  ...
  if (next.legal.inJail) {
    ...
    if (next.legal.jailYearsLeft < 999) {
    ...
    if (next.legal.jailYearsLeft <= 0) {
      ...
      const yearsLeftText = next.legal.jailYearsLeft >= 999 ? "seumur hidup" : `${next.legal.jailYearsLeft} tahun`;
      pushLog(next, `Kamu menjalani hukuman. Sisa masa tahanan: ${yearsLeftText}.`);
    }
  }
  ...
  if (!next.legal.inJail && next.career.jobId) {
    ...
    if (job) {
      // --- RETIREMENT LOGIC (Mandatory at 65) ---
      if (next.age == 65 && !next.career.isRetired && next.career.jobId) {
        ...
        const pensionYearly = Math.floor(salaryWithBoost * 0.6); // 60% of last salary
        const pensionMonthly = Math.floor(pensionYearly / 12);
        ...
        const retirementMsg = `Kamu harus pensiun. Gaji per bulan Rp${pensionMonthly.toLocaleString("id-ID")}.`;
        const logMsg = `Saya harus pensiun. Gaji per bulan Rp${pensionMonthly.toLocaleString("id-ID")}.`;

        pushNotification(next, {
          title: "Pekerjaan",
          message: retirementMsg,
          icon: "success"
        });

        next.stats.happy = clamp(next.stats.happy + 20); // From updatea(20, 0, 0, 0)
        pushLog(next, logMsg);
      } else {
  ...
  if (next.life.isAlive && next.age >= 60) {
    ...
    if (next.age >= 90) baseChance = 0.15;
    else if (next.age >= 85) baseChance = 0.1;
  ...
  if (!next.legal.inJail && next.life.isAlive) {
    ...
    if (next.education.level !== "none") {
      ...
      if (edu) {
        ...
        const maxYears = edu.yearsToComplete || 3; // Use catalog value

        if (isBasicSchool) {
          next.education.classGroup = ["A", "B", "C", "D", "E", "F", "G", "H"][Math.floor(rng() * 8)];
        } else {
          next.education.classGroup = "";
        }
        ...
        if (next.education.yearsStudied < maxYears) {
          ...
          if (isUni) {
            ...
            pushLog(next, `Saya sedang menjalani program ${edu.name} tahun ke-${next.education.yearsStudied}.`);
          } else {
        ...
        // Graduation for University or Kejar Paket
        if ((isUni || isPaket) && next.education.yearsStudied >= maxYears) {
          ...
          if (isUni) {
            ...
            const gradOptions = [];
            if (hasJob) {
              gradOptions.push({ id: "independent", label: "Fokus Karir & Hidup Mandiri", color: "blue" });
            } else {
            ...
            const setaraName = next.education.level === "paket_a" ? "SD" : (next.education.level === "paket_b" ? "SMP" : "SMA");
            const gradMsg = `Selamat! Kamu berhasil menyelesaikan ${eduName}! Sekarang kamu memiliki ijazah setara ${setaraName}.`;
            pushLog(next, gradMsg);

            if (next.education.level === "paket_a") {
              const options = [
              ...
              pushNotification(next, {
                ...
                icon: "success",
                type: "confirm",
                eventId: "graduation_paket_a_selection",
                options: options
              });
              otherEvents = true;
            } else if (next.education.level === "paket_b") {
              const options = [
              ...
              pushNotification(next, {
                ...
                icon: "success",
                type: "confirm",
                eventId: "graduation_paket_b_selection",
                options: options
              });
              otherEvents = true;
            } else {
              ...
              let waitMsg = "";
              if (next.age < 18) {
                waitMsg = " Namun, kamu harus menunggu hingga usia 18 tahun untuk dapat mendaftar kuliah.";
              } else {
                waitMsg = " Sekarang kamu sudah bisa mendaftar kuliah melalui jalur SNBT atau Mandiri!";
              }
  ...
  if (!next.profile.isIndependent && next.life.isAlive && isAnyParentAlive) {
    ...
    if (next.age <= 25) {
      if (rng() < 0.08) {
        ...
        if (isMotherAlive) {
          ...
          repairs.push({ msg: "Kulkas keluarga rusak, Ibu langsung membelinya yang baru agar makanan tidak basi.", cost: 2_000_000 });
        }
        ...
        for (let i = 0; i < count; i++) {
          ...
          pushNotification(next, {
            title: "Masalah Rumah Tangga",
            message: `${picked.msg} (Tabungan berkurang Rp${picked.cost.toLocaleString("id-ID")})`,
            icon: "warning"
          });
          otherEvents = true;
        }
    ...
    ["Bapak", "Ibu"].forEach(person => {
      ...
      if (isAlive && rng() < sicknessChance) {
        ...
        next.family.savings -= cost;
        next.stats.happy = clamp(next.stats.happy - 10);
        const sickMsg = `${person} ku jatuh sakit dan harus dirawat di rumah sakit. Biaya berobat sebesar Rp${cost.toLocaleString("id-ID")} ditanggung keluarga.`;
        pushLog(next, sickMsg);
        pushNotification(next, {
          title: `${person} Sakit`,
          message: sickMsg,
          icon: "warning"
    ...
    if (!next.family.isBankrupt && next.family.wealthStatus !== "poor") {
      if (rng() < 0.03) {
        ...
        if (availableParents.length > 0) {
          ...
          const phkMsg = `${person} mu terkena PHK mendadak akibat perusahaannya bangkrut. Ekonomi keluargamu kini dalam krisis besar.`;
          pushLog(next, phkMsg);
          pushNotification(next, {
            title: "PHK Mendadak!",
            message: phkMsg,
      ...
      if (rng() < 0.15) {
        ...
        if (availableParents.length > 0) {
          ...
          if (isSerabutan) {
            ...
            next.stats.happy = clamp(next.stats.happy + 10);
            recoveryMsg = `${person} mu akhirnya mendapat pekerjaan baru, meski gajinya lebih kecil dari sebelumnya. Paling tidak keluarga kita bisa bernapas lega.`;
          } else {
            next.family.monthlyIncome = 8_000_000 + Math.floor(rng() * 5_000_000);
            next.stats.happy = clamp(next.stats.happy + 20);
            recoveryMsg = `Kabar gembira! ${person} mu diterima bekerja di perusahaan besar dengan gaji yang mapan. Masa krisis telah berlalu.`;
          }
          pushLog(next, recoveryMsg);
        }
      }
    }
  }
